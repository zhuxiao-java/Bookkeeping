...
defineProps<{ summary: CurrencySummary }>()
...
<template>
  <section>
    ...
    <el-empty v-if="!summary.categories.length" description="本月没有消费记录，不生成削减建议" :image-size="72" />
    <el-collapse v-else>
      <el-collapse-item v-for="c in all ? summary.categories : summary.categories.slice(0, 5)" :key="c.categoryId" :name="c.categoryId">
        ...
        <p>平均单笔 {{ c.average }} · {{ comparison(c.amount, c.previousAmount, c.growth) }}</p>
        <p>上月 {{ c.previousAmount ?? '无记录' }} / {{ c.previousCount ?? '—' }} 笔 / 平均 {{ c.previousAverage ?? '—' }} · 前三月均值 {{ c.historyAverage ?? '历史不足' }}</p>
