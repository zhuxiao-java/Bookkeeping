...
const api = vi.hoisted(() => ({
  ...
  list: vi.fn(), detail: vi.fn(), confirm: vi.fn(), info: vi.fn(), leave: vi.fn(), update: vi.fn()
}))
vi.mock('@/api/monthlyReport', () => ({ monthlyAiApi: api, monthlyReportApi: { list: api.list, detail: api.detail } }))
...
vi.mock('vue-router', () => ({ useRoute: () => route, useRouter: () => ({ replace: vi.fn(), push: vi.fn() }), onBeforeRouteLeave: api.leave, onBeforeRouteUpdate: api.update }))
...
const preview: AiPreview = { month: '2024-01', sourceHash: 'hash', baseVersion: 0, configVersion: 'v2', baseUrl: 'https://receiver.example/v1', model: 'preview-model', includeNames: false, summary: { count: 1 } }
function detail(id = 1): MonthlyDetail {
  return { id, month: id === 1 ? '2024-01' : '2024-02', version: 1, generatedAt: '2024-03-01T00:00:00', stale: false,
...
async function mount(component: Component) {
  ...
  for (const name of ['ElForm', 'ElFormItem', 'ElSelect', 'ElCollapse', 'ElCollapseItem', 'ElTag', 'ElIcon', 'ElSkeleton']) app.component(name, box)
...
beforeEach(() => {
  ...
  api.list.mockResolvedValue([{ id: null, month: '2024-01', version: null, hasReport: false }, { id: 2, month: '2024-02', version: 1, hasReport: true }])
  api.generate.mockResolvedValue(detail())
  api.detail.mockImplementation((id: number) => Promise.resolve(detail(id)))
})
...
function existingReport() {
  api.list.mockResolvedValue([{ id: 1, month: '2024-01', version: 1, hasReport: true }])
}

describe('月报手动同步生成', () => {
  it('首次无需本地报告，同一次预览确认后只发送一次，直接展示成功响应', async () => {
    const pending = deferred<MonthlyDetail>(); api.generate.mockReturnValue(pending.promise)
    ...
    expect(api.preview).toHaveBeenCalledWith('2024-01')
    ...
    expect(api.generate).toHaveBeenCalledWith({ month: '2024-01', sourceHash: 'hash', baseVersion: 0, configVersion: 'v2', confirmed: true }, expect.any(AbortSignal))
    expect(root.textContent).toContain('正在整理你的月度故事')
  ...
  it('月份切换丢弃迟到预览', async () => {
    const pending = deferred<AiPreview>(); api.preview.mockReturnValue(pending.promise)
  ...
  it('读取期间明确显示加载状态，不提前显示未配置', async () => {
    const pending = deferred<MonthEntry[]>()
  ...
  it('无数据不调用模型，纯本地历史快照不冒充 AI 报告', async () => {
    ...
    await click('生成 AI 月报')
    expect(root.textContent).toContain('这个月还没有可分析的流水'); expect(api.generate).not.toHaveBeenCalled()
  })
  ...
  it('发送前的详情刷新迟到不能覆盖新生成结果', async () => {
    ...
    const pending = deferred<MonthlyDetail>(); api.detail.mockReturnValueOnce(pending.promise)
